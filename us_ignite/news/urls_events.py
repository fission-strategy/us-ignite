...
urlpatterns = [
    url("^feeds/(?P<format>.*)%s$" % _slash,
        views.news_post_feed,{'events_only': True},  name="news_post_feed"),
    url("^tag/(?P<tag>.*)/feeds/(?P<format>.*)%s$" % _slash,
        views.news_post_feed, name="news_post_feed_tag"),
    url("^tag/(?P<tag>.*)%s$" % _slash,
        views.news_post_list, {'events_only': True}, name="news_post_list_tag"),
    ...
    url("^category/(?P<category>.*)%s$" % _slash,
        views.news_post_list, {'events_only': True}, name="news_post_list_category"),
    ...
    url("^author/(?P<username>.*)%s$" % _slash,
        views.news_post_list, {'events_only': True}, name="news_post_list_author"),
    url("^archive/(?P<year>\d{4})/(?P<month>\d{1,2})%s$" % _slash,
        views.news_post_list, {'events_only': True}, name="news_post_list_month"),
    url("^archive/(?P<year>\d{4})%s$" % _slash,
        views.news_post_list, {'events_only': True}, name="news_post_list_year"),
